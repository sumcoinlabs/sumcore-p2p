...
function builder(options) {
  ...
  exported.inventoryCommands.forEach(function(command) {
    ...
    exported.commands[command].forTransaction = function forTransaction(hash, fWitness) {
      return new exported.commands[command]([Inventory.forTransaction(hash, fWitness)]);
    };

    // add forBlock methods
    exported.commands[command].forBlock = function forBlock(hash, fWitness) {
      return new exported.commands[command]([Inventory.forBlock(hash, fWitness)]);
    };

    // add forFilteredBlock methods
    exported.commands[command].forFilteredBlock = function forFilteredBlock(hash, fWitness) {
      return new exported.commands[command]([Inventory.forFilteredBlock(hash, fWitness)]);
    };
